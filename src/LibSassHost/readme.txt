

   --------------------------------------------------------------------------------
                     README file for LibSass Host for .NET v1.5.0

   --------------------------------------------------------------------------------

           Copyright (c) 2015-2024 Andrey Taritsyn - http://www.taritsyn.ru


   ===========
   DESCRIPTION
   ===========
   .NET wrapper around the LibSass (https://github.com/sass/libsass) with the
   ability to support a virtual file system.

   This package does not contain the native implementations of LibSass. Therefore,
   you need to choose and install the most appropriate package(s) for your platform.
   The following packages are available:

    * LibSassHost.Native.win-x86
    * LibSassHost.Native.win-x64
    * LibSassHost.Native.win-arm
    * LibSassHost.Native.win-arm64
    * LibSassHost.Native.linux-x64
    * LibSassHost.Native.osx-x64

   =============
   RELEASE NOTES
   =============
   Added a packages that contain a native assemblies for Windows (ARM) and (ARM64).

   ============
   PROJECT SITE
   ============
   http://github.com/Taritsyn/LibSassHost